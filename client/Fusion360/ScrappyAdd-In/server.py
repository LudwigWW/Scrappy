...
max_conns = 10
host = ''
port = 47556 # ports we use: 47002-47099
...
path_to_matryoshka = "path/to/matryoshka/Release/matryoshka"
...
def parse_matryoshka_info(info_string):
    try:
        info_string = info_string.split("<!..>")[1] # 0=before, 1=data, 2=after #TODO: Might fail without Matryoshka_Debug flag
    except:
        return []
    info_string = info_string.replace("  ", " ").replace("  ", " ").replace("> ", ">")
...
def acquire_port():
    port_mutex.acquire()
    try:
        file_port = random.randrange(47101,47556,1)
...
def threaded_server(conn, address):
    ...
    file_base_path = "/path/to/debug/folder/IN/"
    file_base_path_send = "/path/to/debug/folder/"
    ...
    while True:
        ...
        # handle sending a file to client after the handshake was successful
            ...
            path_to_file = file_base_path_send + file_name
            print("Sending file to: " + client_host + ":" + data_array[2])
        ...
        # handle request to run matryoshka
            ...
            path_to_outer_file = data_array[2]
            path_to_inner_file = data_array[3]
            print("Executing Matryoshka")
            
            shell_command = path_to_matryoshka + " " + options_string + " " + path_to_outer_file + " " + path_to_inner_file
            command_result = execute_command(shell_command)

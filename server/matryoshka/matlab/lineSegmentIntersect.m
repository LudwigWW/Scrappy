function [B,u_a,u_b,X,Y,P,C] = lineSegmentIntersect(XY1,XY2)
...
X4_X3 = bsxfun(@minus,XY2(:,3)',XY2(:,1)');
Y1_Y3 = bsxfun(@minus,XY1(:,2) ,XY2(:,2)');
Y4_Y3 = bsxfun(@minus,XY2(:,4)',XY2(:,2)');
X1_X3 = bsxfun(@minus,XY1(:,1) ,XY2(:,1)');
X2_X1 = bsxfun(@minus,XY1(:,3) ,XY1(:,1));
Y2_Y1 = bsxfun(@minus,XY1(:,4) ,XY1(:,2));

numerator_a = bsxfun(@times,X4_X3,Y1_Y3) - bsxfun(@times,Y4_Y3,X1_X3);
numerator_b = bsxfun(@times,X2_X1,Y1_Y3) - bsxfun(@times,Y2_Y1,X1_X3);
denominator = bsxfun(@times,Y4_Y3,X2_X1) - bsxfun(@times,X4_X3,Y2_Y1);

u_a = numerator_a ./ denominator;
u_b = numerator_b ./ denominator;

% Find the adjacency matrix A of intersecting lines.
B = (u_a >= 0) & (u_a <= 1) & (u_b >= 0) & (u_b <= 1);
if nargout > 3
  X = B.*(XY1(:,1)+X2_X1.*u_a);
  Y = B.*(XY1(:,2)+Y2_Y1.*u_a);
  P = denominator == 0;
  C = (numerator_a == 0 & numerator_b == 0 & P);
end
